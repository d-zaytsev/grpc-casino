...
export default function Home() {
  ...
  const [user, setUser] = useState({ username: '', password: '', balance: 0, isLoggedIn: false })
  ...
  const handleAuth = (action, username, password) => {
    ...
    callback(username, password, (err, resp) => {
      if (err) {
        console.error(`Ошибка ${action === 'login' ? 'авторизации' : 'регистрации'}:`, err.message)
        setUser({ username: '', password: '', balance: 0, isLoggedIn: false })
      } else {
        console.log(resp.u);
        setUser({
          username,
          password,
          balance: Number(resp.u?.[2]?.[2] || 0),
          isLoggedIn: true
        })
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500">
      ...
      <div className="fixed top-6 right-6 z-10">
        {user.isLoggedIn ? (
          ...
          <button
            onClick={() => setModalDialog(prev => ({ ...prev, login: true }))}
            className="px-6 py-2 bg-white text-purple-600 rounded-lg font-semibold shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200"
          >
            Войти
          </button>
      ...
      <div className="flex items-center justify-center min-h-screen px-4">
        <div className="flex flex-col items-center text-center text-white max-w-2xl">
          ...
          <p className="text-2xl mb-6">
            {user.isLoggedIn ? `Привет, ${user.username}! Как насчёт депа?` : 'Войдите, чтобы продолжить'}
          </p>
          {user.isLoggedIn && (
            <button
              onClick={() => setModalDialog(prev => ({ ...prev, deposit: true }))}
      ...
      <LoginModal
        isOpen={modal.login}
        onClose={() => setModalDialog(prev => ({ ...prev, login: false }))}
      ...
      <DepositModal
        ...
        onClose={() => setModalDialog(prev => ({ ...prev, deposit: false }))}
